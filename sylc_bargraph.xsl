    ...
    <xsl:template match="/">
        <svg>
            <g transform="translate(30, 330)">
                <line x1="{20}" x2="{count(//sylc)*$barInterval}" y1="-{$barHeight div 2}"
                    y2="-{$barHeight div 2}" stroke="black" opacity="0.5" stroke-dasharray="8 4"
                    stroke-width="1"/>
                <line x1="{20}" x2="{count(//sylc)*$barInterval}" y1="-{$barHeight div 4}"
                    y2="-{$barHeight div 4}" stroke="black" opacity="0.5" stroke-dasharray="8 4"
                    stroke-width="1"/>
                <line x1="{20}" x2="{count(//sylc)*$barInterval}" y1="-{$barHeight div 1.25}"
                ...
                <xsl:apply-templates select="//sylc"/>
            </g>
        </svg>
    </xsl:template>
    <xsl:template match="sylc">
