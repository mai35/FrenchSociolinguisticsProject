...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg"
    ...
    <xsl:template match="/">
        <svg>
            <g transform="translate(30, 330)">
                <line x1="{20}" x2="{count(//pos)*$barInterval}" y1="-{$barHeight div 2}"
                    y2="-{$barHeight div 2}" stroke="black" opacity="0.5" stroke-dasharray="8 4"
                    stroke-width="1"/>
                <line x1="{20}" x2="{count(//pos)*$barInterval}" y1="-{$barHeight div 4}"
                    y2="-{$barHeight div 4}" stroke="black" opacity="0.5" stroke-dasharray="8 4"
                    stroke-width="1"/>
                <line x1="{20}" x2="{count(//pos)*$barInterval}" y1="-{$barHeight div 1.25}"
                    y2="-{$barHeight div 1.25}" stroke="black" opacity="0.5" stroke-dasharray="8 4"
                    stroke-width="1"/>
                <line x1="20" x2="20" y1="0" y2="-320" stroke="black" stroke-width="2"/>
                <line x1="20" x2="{7 * $barInterval}" y1="0" y2="0" stroke="black"
                    stroke-width="2"/>
    ...
    <xsl:template match="pos">
        ...
        <xsl:variable name="POS" select="@cat"/>
